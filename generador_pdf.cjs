...
const base_dir = 'c:/Users/pablo/Desktop/nexotalentos';
const logo_path = path.join(base_dir, 'public', 'icon-dark.png');
...
const css_common = `
    <style>
        ...
        h1 { color: #082041; font-size: 22px; margin: 0; letter-spacing: -0.5px; }
        h2 { color: #00A9A3; font-size: 15px; margin-top: 25px; border-bottom: 1px solid #e2e8f0; padding-bottom: 4px; }
        h3 { color: #082041; font-size: 12px; margin-top: 15px; margin-bottom: 5px; }
        ...
        .badge { display: inline-block; padding: 2px 7px; border-radius: 10px; font-size: 8px; font-weight: bold; color: white; margin-right: 6px; }
        .badge-critico { background-color: #ef4444; }
        .badge-advertencia { background-color: #f59e0b; }
        .badge-ok { background-color: #10b981; }
        .total-row td { font-weight: bold; background-color: #e2e8f0; font-size: 11px; }
    </style>
`;
...
const html_dossier = `
<!DOCTYPE html><html><head><meta charset="utf-8">${css_common}</head><body>
    ${header_html}
    <h2 style="text-align: center; color: #082041; font-size: 18px; border: none; margin-bottom: 20px;">DOSSIER TÉCNICO DEL PROYECTO</h2>
...
const html_auditoria = `
<!DOCTYPE html><html><head><meta charset="utf-8">${css_common}</head><body>
    ${header_html}
    <h2 style="text-align: center; color: #082041; font-size: 18px; border: none; margin-bottom: 20px;">AUDITORÍA TÉCNICA Y DE SEGURIDAD EXHAUSTIVA</h2>
...
const html_valoracion = `
<!DOCTYPE html><html><head><meta charset="utf-8">${css_common}</head><body>
    ${header_html}
    <h2 style="text-align: center; color: #082041; font-size: 18px; border: none; margin-bottom: 20px;">VALORACIÓN ECONÓMICA Y TIEMPOS REALES</h2>
    ...
    <div style="margin-top: 25px; padding: 14px; border-left: 4px solid #082041; background-color: #f1f5f9; font-size: 11px;">
...
async function generatePDFs() {
    ...
    await page.setContent(html_dossier);
    await page.pdf({ path: '1_Dossier_Proyecto.pdf', format: 'A4', margin: { top: '20mm', bottom: '20mm', left: '15mm', right: '15mm' } });
    
    await page.setContent(html_auditoria);
    await page.pdf({ path: '2_Auditoria_Tecnica.pdf', format: 'A4', margin: { top: '20mm', bottom: '20mm', left: '15mm', right: '15mm' } });
    
    await page.setContent(html_valoracion);
    await page.pdf({ path: '3_Valoracion_Economica.pdf', format: 'A4', margin: { top: '20mm', bottom: '20mm', left: '15mm', right: '15mm' } });
    
    await browser.close();
    console.log('¡PDFs actualizados y generados con éxito!');
}
